...
import com.baomidou.mybatisplus.generator.entity.TestEntity;
 ...
 * @since 3.5.3
 */
public class JavaAnnotationCodeGeneratorTest extends BaseGeneratorTest {
     ...
     * 数据源配置
     */
    private static final DataSourceConfig DATA_SOURCE_CONFIG = new DataSourceConfig.Builder("jdbc:h2:mem:test;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE;CASE_INSENSITIVE_IDENTIFIERS=TRUE;MODE=MYSQL", "sa", "").databaseQueryClass(SQLQuery.class) // 设置SQL查询方式，默认的是元数据查询方式
        .build();
    ...
    public void testSimple() {
        ...
        Class<?> testEntityClass = TestEntity.class;

        GlobalConfig globalConfig = globalConfig().disableOpenDir().outputDir(tempDir.getAbsolutePath()).build();

        TableName annotation = testEntityClass.getAnnotation(TableName.class);
        if (annotation == null) {
            return;
        }

        String packageName = testEntityClass.getPackage().getName();

        String moduleName = "test-info";
        PackageConfig packageConfig = packageConfig().moduleName(moduleName).parent(packageName).build();

        TableInfo tableInfo = new TableInfo(null, annotation.value());
        tableInfo.setHavePrimaryKey(false);

        tableInfo.processTable();

        //这个是必须参数,其他都是可选的,后续去除默认构造更改成final
        ConfigBuilder configBuilder = new ConfigBuilder(packageConfig, GeneratorBuilder.strategyConfig(), null, globalConfig, null);
        ...
        // 备注信息
        AutoGenerator generator = new AutoGenerator(configBuilder);
    ...
    public void testVersionAndFill() {
        AutoGenerator generator = new AutoGenerator(DATA_SOURCE_CONFIG);
        generator.strategy(strategyConfig().entityBuilder().versionColumnName("version") // 基于数据库字段
    ...
    public void testLogicDeleteAndIgnoreColumn() {
        AutoGenerator generator = new AutoGenerator(DATA_SOURCE_CONFIG);
        generator.strategy(strategyConfig().entityBuilder().logicDeleteColumnName("deleted") // 基于数据库字段
    ...
    public void testCustomFileByLambda() {
        ...
        AutoGenerator generator = new AutoGenerator(DATA_SOURCE_CONFIG);
        generator.strategy(strategyConfig().build());
        generator.injection(injectionConfig().customMap(map).customFile(file -> file.fileName("DTO.java").templatePath("/templates/dto.java.vm").filePath("D://")).customFile(file -> file.fileName("VO.java").templatePath("/templates/vo.java.vm").enableFileOverride().filePath("D://")).build());
        generator.global(globalConfig().build());
        generator.execute();
    }
}
